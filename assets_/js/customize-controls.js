/* global MayLoeBgColors, MayLoeColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( MayLoeBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( MayLoeBgColors[ context ].color ) {
						backgroundColorValue = MayLoeBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( MayLoeBgColors[ context ].setting ).get();
					}
					MayLoeSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( MayLoeBgColors ).forEach( function( context ) {
			wp.customize( MayLoeBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					MayLoeSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		MayLoeSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				MayLoeSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function MayLoeSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = MayLoeColor( backgroundColor, accentHue );
	 ...
	 * @return {void}
	 */
	function MayLoeSetRetineLogoVisibility( visible ) {
