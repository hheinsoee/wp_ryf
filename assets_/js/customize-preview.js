/* global MayLoeBgColors, MayLoePreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( MayLoeBgColors ).forEach( function( context ) {
					MayLoeGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function MayLoeGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'MayLoe-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#MayLoe-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( MayLoePreviewEls[ context ] ) {
				_.each( MayLoePreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		MayLoeGenerateColorA11yPreviewStyles( 'content' );
		MayLoeGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
